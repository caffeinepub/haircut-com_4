...
import { ChevronRight, MapPin, SlidersHorizontal, Star } from "lucide-react";
...
export function SearchResults() {
  ...
  const { data: salons, isLoading } = useSearchSalons({
    category,
    minRating,
    openNow,
    city,
  ...
  useEffect(() => {
    ...
    setPageTitle(title);
    setMetaDescription(
      `Find and book the best ${category || ""} salons ${city ? `in ${city}` : "near you"} on Haircut.com.`,
    );
  }, [category, city]);
    ...
    >
      ...
      <div className="p-4 space-y-2">
        <div className="flex items-start justify-between gap-2">
          <h3 className="font-semibold text-foreground line-clamp-1 group-hover:text-gold-500 transition-colors">
            {salon.name}
          </h3>

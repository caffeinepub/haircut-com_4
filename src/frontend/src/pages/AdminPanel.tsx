  ...
  return (
    <div className="mb-4 p-4 rounded-xl bg-yellow-500/10 border border-yellow-500/30 flex items-center gap-3">
      ...
      <p className="text-sm text-yellow-700 dark:text-yellow-400">
        Stripe is not configured. Go to the <strong>Stripe</strong> tab to set
        up payments.
      </p>
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      <h2 className="font-semibold text-foreground">
        Pending Salon Approvals ({salons?.length || 0})
      </h2>
      {salons?.length === 0 ? (
        ...
        salons?.map((salon) => (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {salon.ownerName} · {salon.category}
                </p>
            ...
            <div className="text-xs text-muted-foreground">
              Services:{" "}
              {salon.services.map((s) => s.name).join(", ") || "None listed"}
            </div>
            <Input
              value={rejectReasons[salon.id] || ""}
              onChange={(e) =>
                setRejectReasons((p) => ({ ...p, [salon.id]: e.target.value }))
              }
...
function CommissionSettings() {
  ...
  useEffect(() => {
    if (rates) {
      ...
      for (const [k, v] of Object.entries(rates.byCategory)) {
        cr[k] = String(v);
      }
      setCategoryRates(cr);
    }
  ...
  const handleSave = async () => {
    ...
    for (const [k, v] of Object.entries(categoryRates)) {
          ...
          {["Men", "Women", "Kids", "Unisex"].map((cat) => (
            <div key={cat}>
              ...
              <Input
                ...
                onChange={(e) =>
                  setCategoryRates((p) => ({ ...p, [cat]: e.target.value }))
                }
                placeholder="10"
                className="rounded-xl"
              />
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          {
            label: "GMV",
            value: `₹${(analytics.gmv / 100000).toFixed(1)}L`,
            icon: TrendingUp,
          },
          {
            label: "Revenue",
            value: `₹${(analytics.platformRevenue / 1000).toFixed(0)}K`,
            icon: DollarSign,
          },
          {
            label: "Cancellation",
      ...
      <div className="bg-card border border-border rounded-2xl p-5">
        ...
        <ResponsiveContainer width="100%" height={200}>
          <PieChart>
            ...
            >
              {analytics.topSalons.map((salon, i) => (
                <Cell
                  key={salon.name}
    ...
    <div className="space-y-6">
      ...
      {isConfigured ? (
        <div className="bg-green-500/10 border border-green-500/30 rounded-2xl p-5 text-center space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            Payments are being processed through Stripe.
          </p>
        </div>
      ) : (
        <div className="bg-card border border-border rounded-2xl p-5 space-y-4">
          <p className="text-sm text-muted-foreground">
            Configure Stripe to enable online payments. Get your secret key from
            the{" "}
            <a
